...
def get_relation_constraints() -> List[BaseRelationConstraint]:
    ...
    # g7: if x[:, 19] > 0 then x[:,25] > 0
    g7 = apply_if_a_supp_zero_than_b_supp_zero(Feature(19), Feature(25))

    # g8: if x[:, 2] > 0 then x[:,25] > 0
    g8 = apply_if_a_supp_zero_than_b_supp_zero(Feature(2), Feature(25))

    # g9: if x[:, 2] > 0 then x[:,25] > 0
    g9 = apply_if_a_supp_zero_than_b_supp_zero(Feature(2), Feature(25))
