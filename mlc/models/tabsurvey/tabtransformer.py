...
from mlc.transformers.tab_scaler import ScalerData, TabScaler
...
class ScalerModule(nn.Module):
    def __init__(self, scaler: TabScaler):
        ...
        if scaler.one_hot_encode:
            raise ValueError(
                f"One hot encoding " f"not supported for TabTransformer"
            )

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if (not self.scaler.do_one_hot_encode()) and (
            len(self.scaler.cat_idx) > 0
        ):
        ...
        # print(f"scaler x_categ: {x_categ.shape}")
        return (
            x_categ,
            x_cont,
        )
...
class TabTransformer(BaseModelTorch):
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger = logging.getLogger(__name__)
        logger.debug(f"Cat idx: {self.cat_idx}")
        ...
        if scaler is not None:
            logger.debug("Using provided scaler")
        ...
        if self.scaler is not None:
            logger.debug("Using provided scaler")
            self.model = nn.Sequential(ScalerModule(self.scaler), self.model)
        else:
            logger.warning("No scaler! fit method may fail")
    ...
    def save(self, path: str, train_search=False) -> None:
        ...
        if self.scaler is not None and not train_search:
            if self.scaler_type == "CTGAN":
                scaler_path = f"{path}/scaler.joblib"
                joblib.dump(self.scaler, parent_exists(scaler_path))
            else:
    ...
    def load_class(cls, path: str, **kwargs: Dict[str, Any]) -> BaseModelTorch:
        ...
        # model.model.load_state_dict(torch.load(weigths_path, map_location=torch.device("cpu")))
        return model
    ...
    def fit(
        ...
        for epoch in range(self.epochs):
            ...
            for i, (batch_x, batch_y) in enumerate(train_loader):
                logger.debug(f"batch_x: {type(batch_x)}")
                logger.debug(f"model type: {type(self.model)}")
                logger.debug(f"batch_shape: {batch_x.shape}")
    ...
    def get_default_params(trial_params: Dict[str, Any]) -> Dict[str, Any]:
        params = {
            ...
            "weight_decay": -6,  # x = 10 ^ u
            "learning_rate": -3,  # x = 10 ^ u
            "dropout": 0.1,
        }
        return params
...
class TabTransformerModel(nn.Module):
    ...
    def forward(
        ...
        logger.debug(f"x_categ: {x_categ}")
        logger.debug(f"x_cont: {x_cont}")
        logger.debug(f"x_cont: {x_cont.shape}")
        # logger.debug(f"x_categ: {x_categ.shape}")
