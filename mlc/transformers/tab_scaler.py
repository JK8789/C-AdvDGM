...
from mlc.utils import round_func_BPDA, to_numpy_number, to_torch_number
...
class ScalerData:
    ...
    def save(self, path: str) -> None:
        out = {
            "x_min": self.x_min.numpy().tolist()
            if self.x_min is not None
            else None,
            "x_max": self.x_max.numpy().tolist()
...
class TabScaler:
    ...
    def inverse_transform(
        ...
        x[:, self.num_idx] = torch.clip(x[:, self.num_idx], self.x_min, self.x_max)
...
def power_encode(coded: torch.Tensor, dummies: torch.Tensor) -> torch.Tensor:
    diff_pos = (coded - dummies.to(coded.device)) ** 2
    diff_neg = 1 - diff_pos
