...
def get_feature_min_max(
    ...
    if np.issubdtype(lower_bounds.dtype, np.number) and np.issubdtype(upper_bounds.dtype, np.number):
        if x.ndim == 1:
            return lower_bounds, upper_bounds
        else:
            return np.tile(lower_bounds, (x.shape[0] ,1)), np.tile(upper_bounds, (x.shape[0] ,1)) 
    else:
