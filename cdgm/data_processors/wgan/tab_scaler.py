...
from cdgm.data_processors.wgan.typing import NDNumber
from cdgm.data_processors.wgan.utils import round_func_BPDA, to_numpy_number
...
def softargmax(x_cat: torch.Tensor, dim: int = -1, dummies=None) -> torch.Tensor:
...
class TabScaler:
    ...
    def transform(
        ...
        if self.num_scaler == "min_max":
            ...
            x_out = (x[:, self.num_idx] - torch.tensor(self.x_min)) / self.min_max_scale
            x[:, self.num_idx] = x_out * (torch.tensor(self.out_max) - torch.tensor(self.out_min)) + torch.tensor(self.out_min)
            # x[:, self.num_idx] = (
        ...
        if self.one_hot_encode:
            ...
            for i, idx in enumerate(self.cat_idx):
                ...
                coded = codes.repeat_interleave(num_categories).reshape(rows, num_categories)
    ...
    def inverse_transform(
        ...
        if self.one_hot_encode:
            ...
            for i, categories in enumerate(self.ohe.categories_):
                ...
                self.torch_dummies = torch.tensor(categories, dtype=torch.float32)
                decoded = softargmax(x[:, start_idx:end_idx], 1, self.torch_dummies)
            ...
            x_out = torch.concat(out, dim=1)
        else:
            x_out = x.clone()
        # Numerical features
        if self.num_scaler == "min_max":
            x_out[:, self.num_idx] = (x_out[:, self.num_idx] - self.out_min) / (
                self.out_max - self.out_min
            )
            x_out[:, self.num_idx] = (
                x_out[:, self.num_idx] * self.min_max_scale + self.x_min
            )

        elif self.num_scaler == "standard":
            x_out[:, self.num_idx] = x_out[:, self.num_idx] * self.std + self.mean
        ...
        x_out[:, self.num_idx] = torch.clip(x_out[:, self.num_idx], self.x_min, self.x_max)
