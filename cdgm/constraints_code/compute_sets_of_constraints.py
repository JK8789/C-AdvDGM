...
from cdgm.constraints_code.classes import Variable, Constraint, Atom, Atom_with_Multiplication, Inequality, Inequality_with_Multiplication
    ...
    for constr in constraints_with_y:
        ...
        for atom in constr.get_body_atoms():
            if isinstance(atom, Atom_with_Multiplication):
                if y.id in [atom.variable1.id, atom.variable2.id]:
                    flag = True
            else:
                if atom.variable.id == y.id:
                    flag = True
...
def main():
    ...
    print([vv.readable() for vv in ordering])
    for key_var in sets_of_constr:
        print('\nSet of constraints for', key_var.readable())
        for cons in sets_of_constr[key_var]:
            print(cons.readable())
    # for var in ordering:
    #     print(var.readable())
