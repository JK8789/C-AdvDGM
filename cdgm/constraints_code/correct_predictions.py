...
from cdgm.constraints_code.classes import Atom_with_Multiplication, Variable, Constraint, eval_atoms_list, get_missing_mask
from cdgm.constraints_code.compute_sets_of_constraints import compute_sets_of_constraints, get_pos_neg_x_constr
from cdgm.constraints_code.parser import parse_constraints_file
from cdgm.constraints_code.feature_orderings import set_random_ordering
import pandas as pd
INFINITY = torch.inf
...
def get_constr_at_level_x(x, sets_of_constr):
    for var in sets_of_constr:
        if var.id == x.id:
            if sets_of_constr[var]==None:
                print(var.id)
                print(sets_of_constr[var])
...
def get_partial_x_correction(x: Variable, x_positive: bool, x_constraints: List[Constraint],
    ...
    for constr in x_constraints:
        ...
        if isinstance(x_atom, Atom_with_Multiplication):
            # take the value of the other variable as coeff for x:
            var_as_coeff = x_atom.variable1 if x_atom.variable1.id == x.id else x_atom.variable2
            x_coeff *= preds[:, var_as_coeff.id]   # TODO: check the shape of this, as it won't be scalar anymore for B > 1
            mask_coeff_zero = x_coeff==0
            x_coeff[mask_coeff_zero] += epsilon  # * x_weight

        else:
            mask_coeff_zero = None
        ...
        # if atom contains multiplication of vars, mask out where the coeff var is 0:
        # if mask_coeff_zero is not None:
        #    evaluated_complement_body[mask_coeff_zero] += INFINITY * x_weight
...
def main():
    ...
    ordering, constraints = parse_constraints_file('../constraints_data/heloc_constraints.txt')
...
def check_all_constraints_are_sat(constraints, preds, corrected_preds):
    ...
    for constr in constraints:
        ...
        if not sat:
            print('Not satisfied BEFORE correction!', constr.readable())
    ...
    for constr in constraints:
        ...
        if not sat:
            ...
            print('Not satisfied AFTER correction!', constr.readable())
