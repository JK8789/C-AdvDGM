...
class Atom_with_Multiplication(Atom):
    def __init__(self, atom1: Atom, atom2: Atom):
        self.variable1 = atom1.variable
        self.variable2 = atom2.variable
        self.coefficient = atom1.coefficient * atom2.coefficient
        self.positive_sign = atom1.positive_sign == atom2.positive_sign

    def get_variable_id(self):
        return [self.variable1.get_variable_id(), self.variable2.get_variable_id()]

    def eval(self, x_values):
        return x_values[0] * x_values[1] * self.get_signed_coefficient()
    ...
    def readable(self):
        ...
        readable += (f'{self.coefficient:.2f}' if self.coefficient != int(
            self.coefficient) else f'{self.coefficient:.0f}') if self.coefficient != 1 else ''
        readable += self.variable1.readable() + ' * ' + self.variable2.readable()
        return readable

    def get_atom_attributes(self):
        return self.variable1, self.variable2, self.coefficient, self.positive_sign
...
class Inequality_with_Multiplication(Inequality):
    def __init__(self, body: List[Atom], multiplication_atoms: List[Tuple[Atom]], ineq_sign: str, constant: float):
        self.ineq_sign = ineq_sign
        self.constant = constant
        self.body = self.get_body(body, multiplication_atoms)

    def get_body(self, body: List[Atom], multiplication_atoms: List[Tuple[Atom]]) -> List[Atom]:
        for multiplication_atom in multiplication_atoms:
            atom = Atom_with_Multiplication(*multiplication_atom)
            body.append(atom)
        return body
    ...
    def get_body_variables(self):
        var_list = []
        for atom in self.body:
            if isinstance(atom, Atom_with_Multiplication):
                var_list.append(atom.variable1)
                var_list.append(atom.variable2)
            else:
                var_list.append(atom.variable)
        return var_list
    ...
    def get_x_complement_body_atoms(self, x: Variable) -> (List[Atom], Atom, bool):
        ...
        # from which x occurrences have been removed
        complementary_atom_list = []
        x_atom_occurrences_in_multiplication = []
        x_atom_occurrences = []
        for atom in self.body:
            if isinstance(atom, Atom_with_Multiplication):
                if atom.variable1.id != x.id and atom.variable2.id != x.id:
                    complementary_atom_list.append(atom)
                else:
                    x_atom_occurrences.append(atom)
                    x_atom_occurrences_in_multiplication.append(atom)
            else:
                if atom.variable.id != x.id:
                    complementary_atom_list.append(atom)
                else:
        ...
        assert len(x_atom_occurrences) <= 1, "variable {x.id} appears more than one time, function collapse_atoms() from compute_sets_of_constraints should be applied"
        if len(x_atom_occurrences) == 1:
            if x_atom_occurrences_in_multiplication != []:
                x_atom_occurrences = x_atom_occurrences_in_multiplication[0]
            else:
                x_atom_occurrences = x_atom_occurrences[0]
        is_strict_inequality = True if self.ineq_sign == '>' else False
        return complementary_atom_list, x_atom_occurrences, self.constant, is_strict_inequality
...
def eval_atoms_list(atoms_list: List[Atom], preds: torch.Tensor, reduction='sum'):
    ...
    for atom in atoms_list:
        if isinstance(atom, Atom_with_Multiplication):
            atom_values = [preds[:, atom.variable1.id], preds[:, atom.variable2.id]]
            evaluated_atom = atom.eval(atom_values)
        else:
            atom_value = preds[:, atom.variable.id]
            evaluated_atom = atom.eval(atom_value)

        evaluated_atoms.append(evaluated_atom)
...
def get_missing_mask(ineq_atoms: List[Atom], preds: torch.Tensor):
    ...
    for atoms_list in ineq_atoms:
        for atom in atoms_list:
            if isinstance(atom, Atom_with_Multiplication):
                raw_variable_values.append(preds[:, atom.variable1.id])
                raw_variable_values.append(preds[:, atom.variable2.id])
            else:
