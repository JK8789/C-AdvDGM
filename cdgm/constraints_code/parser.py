...
from cdgm.constraints_code.classes import Variable, Atom, Inequality, Constraint, Inequality_with_Multiplication
...
def parse_inequality(inequality):
    ...
    if '*' in body:
        return parse_inequality_with_multiplication(body, constant, ineq_sign)
    atoms_list = []
    constructed_atom = ''
    for elem in body:
...
def parse_inequality_with_multiplication(body, constant, ineq_sign):
    atoms_list = []
    multiplication_atoms_list = []
    constructed_atom = ''
    second_atom_in_multiplication = False
    for elem in body:
        if elem in ALLOWED_OPS:
            if constructed_atom == '':
                constructed_atom += elem
                ...
                atom = parse_atom(constructed_atom)
                if atom is not None:
                    if not second_atom_in_multiplication:
                        if elem == '*':
                            first_atom_in_multiplication = atom
                            second_atom_in_multiplication = True
                        else:
                            atoms_list.append(atom)
                    else:
                        multiplication_atoms_list.append((first_atom_in_multiplication, atom))
                        second_atom_in_multiplication = False
                constructed_atom = elem if elem != '*' else ''
        else:
            constructed_atom += elem
    atom = parse_atom(constructed_atom)
    if atom is not None:
    ...
    ineq = Inequality_with_Multiplication(atoms_list, multiplication_atoms_list, ineq_sign, constant)
    return ineq
